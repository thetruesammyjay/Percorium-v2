...
Validate every asset against the operator-reviewed mint allowlist. Validate SNS links against the resolved public key. Treat claim links, handles, transaction signatures, and wallet addresses as untrusted input. Keep MagicBlock off by default and expose Base only through an explicit Settings flag.
